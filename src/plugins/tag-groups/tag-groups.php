...
define( "TAG_GROUPS_VERSION", "0.24" );
...
function tg_admin_init()
{
    ...
    // Ajax Handler
    add_action( 'wp_ajax_tg_ajax_manage_groups', 'tg_ajax_manage_groups' );
...
function tg_bulk_admin_footer()
{
    ...
    <?php for ( $i = 0; $i < count( $tag_group_labels ); $i++ ) : ?>
                sel_top.append(jQuery("<option>").attr("value", "<?php echo $tag_group_ids[$i] ?>").text("<?php echo htmlentities( $tag_group_labels[$i] ) ?>"));
                sel_bottom.append(jQuery("<option>").attr("value", "<?php echo $tag_group_ids[$i] ?>").text("<?php echo htmlentities( $tag_group_labels[$i] ) ?>"));
    <?php endfor; ?>
...
function tg_bulk_admin_notices()
{
    ...
    if ( isset( $_REQUEST['number_assigned'] ) && (int) $_REQUEST['number_assigned'] && isset( $_REQUEST['group_id'] ) ) {
        ...
        if ( $_REQUEST['group_id'] == 0 ) {
            ...
            $group_name = htmlentities( $tag_group_labels[$i] );
 ...
 * Adds css to backend
 */
function tg_add_admin_js_css( $where )
{

    if ( 'posts_page_tag-groups' == $where ) {

        wp_register_style( 'tag-groups-css-backend', plugins_url( 'css/style.css', __FILE__ ) );

        wp_enqueue_style( 'tag-groups-css-backend' );

        wp_register_script( 'tag-groups-js-backend', plugins_url( 'js/taggroups.js', __FILE__ ) );
            ...
            <label><span class="title"><?php _e( 'Group', 'tag-groups' ) ?></span><span class="input-text-wrap">
                    ...
                    <select id="term-group-option" name="term-group-option" class="ptitle">
                        ...
                        <?php for ( $i = 1; $i <= $number_of_tag_groups; $i++ ) : ?>
                            ...
                            <option value="<?php echo $tag_group_ids[$i]; ?>" ><?php echo htmlentities( $tag_group_labels[$i] ); ?></option>
            ...
            <select id="term-group" name="term-group">
                ...
                <?php for ( $i = 1; $i <= $number_of_tag_groups; $i++ ) : ?>
                    ...
                    <option value="<?php echo $tag_group_ids[$i]; ?>"
                            ...
                            ?> ><?php echo htmlentities( $tag_group_labels[$i] ); ?></option>
...
function tg_group_administration()
{
    ...
    <div class='wrap'>
        ...
        <table class="widefat striped tg_groups_table">
            <thead>
                <tr>
                    <th style="min-width:30px;"><?php _e( 'ID', 'tag-groups' ) ?></th>
    ...
    <script>
        ...
        labels.edit = '<?php _e( 'Edit', 'tag-groups' ) ?>';
        labels.create = '<?php _e( 'Create', 'tag-groups' ) ?>';
        labels.new = '<?php _e( 'new', 'tag-groups' ) ?>';
        labels.placeholder_new = '<?php _e( 'label', 'tag-groups' ) ?>';
        labels.tooltip_delete = '<?php _e( 'Delete this group.', 'tag-groups' ) ?>';
        labels.tooltip_newbelow = '<?php _e( 'Create a new group below.', 'tag-groups' ) ?>';
        labels.tooltip_move_up = '<?php _e( 'move up', 'tag-groups' ) ?>';
        labels.tooltip_move_down = '<?php _e( 'move down', 'tag-groups' ) ?>';
        labels.tooltip_reload = '<?php _e( 'reload', 'tag-groups' ) ?>';

        var tg_params = {"ajaxurl": "<?php
    $protocol = isset( $_SERVER['HTTPS'] ) ? 'https://' : 'http://';
    echo admin_url( 'admin-ajax.php', $protocol );
    ?>", "items_per_page": "<?php echo TAG_GROUPS_ITEMS_PER_PAGE ?>"};
        var data = {};
        ...
        jQuery(document).ready(function () {
            ...
            jQuery(".tg_edit_label_no").live('click', function () {
                ...
                var span = element.parent();
                var input = span.children(":first");
                tg_close_textfield(element.parent(), false);
            });
            ...
            jQuery(".tg_delete").live('click', function () {
                var element = jQuery(this);
                element.parent().parent().addClass('tg_ask_delete');
                var answer = confirm('<?PHP _e( 'Do you really want to delete this tag group?', 'tag-groups' ) ?> ');
                if (answer) {
                    var position = element.attr("data-position");
                    ...
                    element.parent().parent().removeClass('tg_ask_delete')
                }
            });
            ...
            jQuery(".tg_pager_button").live('click', function () {
                ...
                jQuery("#tg_start_position").val((page - 1) * <?php echo TAG_GROUPS_ITEMS_PER_PAGE ?> + 1);
                data.task = "refresh";
                tg_do_ajax(tg_params, data, labels);
            });
            ...
            jQuery("#tg_groups_container").sortable({
                ...
                update: function (event, ui) {
                    ...
                    data.task = "move";
                    data.end_position = element + end_pos - start_pos;
                    tg_do_ajax(tg_params, data, labels);
                }
            });
 ...
 * 
 */
function tg_ajax_manage_groups()
{
    $tag_group_labels = get_option( 'tag_group_labels', array() );
    ...
    if ( (current_user_can( 'manage_options' ) && wp_verify_nonce( $_REQUEST['nonce'], 'tg_groups_management' )) ||
            $task == 'refresh' ) {
        ...
        if ( isset( $_REQUEST['end_position'] ) ) {
            $end_position = $_REQUEST['end_position'];
        }
        ...
        switch ( $task ) {
            case "new":
                ...
                if ( empty( $label ) ) {
                    ...
                    send_error( $response, 'Security check', $task );
                    ...
                    // shift to right position, behind the selected group
                    if ( $number_of_tag_groups != $position ) {
                        tg_move( $tag_group_labels, $number_of_tag_groups, $position + 1 );

                        tg_move( $tag_group_ids, $number_of_tag_groups, $position + 1 );
                    }
                    ...
                    $message = sprintf( __( 'A new tag group with the label \'%s\' has been saved!', 'tag-groups' ), $label );
                }
                break;
            ...
            case "update":
                ...
                if ( empty( $label ) ) {
                    $message = __( 'The label cannot be empty.', 'tag-groups' );
                    send_error( $response, $message, $task );
                } elseif ( (is_array( $tag_group_labels )) && (in_array( $label, $tag_group_labels )) ) {
                    if ( !empty( $position ) && $label == $tag_group_labels[$position] ) {
                    ...
                    if ( !empty( $position ) ) {
                        ...
                        tg_unregister_string_wpml( $tag_group_labels[$position] );

                        $tag_group_labels[$position] = $label;

                        tg_register_string_wpml( 'Group Label ID ' . $position, $tag_group_labels[$position] );
                    } else {

                        send_error( $response, 'error: invalid position: ' . $position, $task );
                    }
            ...
            case "delete":
                if ( !empty( $position ) && $position <= $max_tag_group_id ) {

                    $label = $tag_group_labels[$position];

                    $id = $tag_group_ids[$position];

                    array_splice( $tag_group_labels, $position, 1 );

                    array_splice( $tag_group_ids, $position, 1 );
                    ...
                    $message = sprintf( __( 'A tag group with the id %s and the label \'%s\' has been deleted.', 'tag-groups' ), $id, $label );
                } else {
            ...
            case "up":
                if ( ($position > 1 ) ) {
                    ...
                    tg_swap( $tag_group_labels, $position, $position - 1 );

                    tg_swap( $tag_group_ids, $position, $position - 1 );
            ...
            case "down":
                if ( ($position < $number_of_tag_groups ) ) {
            ...
            case "move":
                ...
                if ( $end_position > $number_of_tag_groups ) {
                    $end_position = $number_of_tag_groups;
                }
                ...
                tg_move( $tag_group_labels, $position, $end_position );

                tg_move( $tag_group_ids, $position, $end_position );
        ...
        $number_of_tag_groups = count( get_option( 'tag_group_labels', array() ) ) - 1;

        if ( $start_position > $number_of_tag_groups ) {
            $start_position = $number_of_tag_groups;
        }

        // always beginning of a page
        $start_position = floor( ($start_position - 1) / TAG_GROUPS_ITEMS_PER_PAGE ) * TAG_GROUPS_ITEMS_PER_PAGE + 1;

        if ( $start_position + TAG_GROUPS_ITEMS_PER_PAGE - 1 < $number_of_tag_groups ) {

            $end_position = $start_position + TAG_GROUPS_ITEMS_PER_PAGE - 1;
        } else {
        ...
        $response->add( array(
            ...
            'supplemental' => array(
                ...
                'groups' => json_encode( tg_group_table( $start_position, $end_position ) ),
                'max_number' => $number_of_tag_groups
            ),
                )
        );
    } else {
 ...
 */
function send_error( $response, $message = 'error', $task = 'unknown' )
{
    $response->add( array(
        'data' => 'error',
        'supplemental' => array(
...
/**
 * Assemble the content of the table of tag groups
 */
function tg_group_table( $start_position, $end_position )
{
    ...
    if ( count( $tag_group_labels ) > 1 ) {
        ...
        for ( $i = $start_position; $i <= $end_position; $i++ ) {
            ...
            array_push( $output, array(
                'id' => $tag_group_ids[$i],
                'label' => $tag_group_labels[$i],
                'amount' => tg_number_assigned( $tag_group_ids[$i] )
                    )
            );
        }
    }
...
function tg_settings_page()
{
    ...
    <div class='wrap'>
        ...
        <?php if ( $active_tab == 3 ): ?>
            ...
            <ul>
                ...
                <li><?php _e( '<b>amount=x</b> Maximum amount of tags in one cloud (per group). Default: 0 (unlimited)', 'tag-groups' ) ?></li>
                ...
                <li><?php _e( '<b>custom_title="abc"</b> Custom title shown as tooltip. No HTML, but you can use {count} and {description} as placeholders. Default: empty (using the description, see also <b>show_tag_count</b>)', 'tag-groups' ) ?></li>
                <li>&nbsp;</li>
                ...
                <li><?php _e( '<b>show_not_assigned=1 or =0</b> Whether to show a group with not-assigned tags. Default: 0', 'tag-groups' ) ?></li>
                <li><?php _e( '<b>not_assigned_name="abc"</b> The name used in the tab of not-assigned tags. Default: "not assigned"', 'tag-groups' ) ?></li>
    ...
    <?php if ( $active_tab == 5 ): ?>
        ...
        <p><?php _e( 'This plugin is the result of many years of development, adding new features, fixing bugs and answering to support questions.', 'tag-groups' ) ?></p>
 ...
 * 
 */
function tg_move( &$ary, $from, $to )
{

    $temp = array_splice( $ary, $from, 1 );

    array_splice( $ary, $to, 0, $temp );
    ...
    if ( count( array_intersect( $tg_type, get_object_taxonomies( $type ) ) ) ) {
        ...
        <select name="tg_filter_posts_value">
            ...
            foreach ( $values as $value => $label ) {
                ...
                printf( '<option value="%s"%s>%s</option>', $value, ( $current_v != '' && $value == $current_v ) ? ' selected="selected"' : '', htmlentities( $label ) );
            }
            ?>
        </select>
        <?php
...
function tag_groups_cloud( $atts = array(), $return_array = false )
{
    ...
    if ( $return_array ) {
        ...
        for ( $i = $start_group; $i <= $number_of_tag_groups; $i++ ) {
            ...
            for ( $i = $start_group; $i <= $number_of_tag_groups; $i++ ) {
                ...
                if ( $show_all_groups || empty( $include_array ) || in_array( $tag_group_ids[$i], $include_array ) ) {
                    ...
                    if ( $i == 0 ) {
                        ...
                        $group_name = tg_translate_string_wpml( 'Group Label ID ' . $tag_group_ids[$i], $tag_group_labels[$i] );
                    }
